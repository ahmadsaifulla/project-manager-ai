...
# Base directory for workspace files. Configurable via env var.
BASE_DIR = os.getenv(
    "PROJECT_BASE_DIR",
...
def initialize_workspace(base_dir: str):
    """
...
def run_hidden_backend_pass(user_request: str, affected_layers: List[str], base_dir: str) -> str:
    """
...
def elicit_goals_node(state: ProjectState) -> Dict[str, Any]:
    ...
    architect_notes = run_hidden_backend_pass(last_human_msg, affected_layers, base_dir)
    ...
    pm_result = run_public_conversational_pass(
        messages_history_str, architect_notes, phase, base_dir
    )
